...
class Requester {
  ...
  async send (options) {
    ...
    const host = options.host || params.host || (options.pathToHost ? options.pathToHost(path) : this.pathToHost(path));
    const protocol = options.protocol || params.protocol || this.protocol;
    const port = options.port || params.port || this.port;
